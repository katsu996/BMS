...
同梱の `docs/table/pages_ui_config.json` では、**オブジェクト内のキー順**をトップページの表の **左から右の列順**（難易度表ブロックは `index.html` の `tablePri`、続けてアルファベット順の追加列、DB ブロックは `dbPriFull`、その右に **IR 3 列**（`ir:lr2ir` / `ir:minir` / `ir:mocha`）、最後に Chart 列）に合わせています。
